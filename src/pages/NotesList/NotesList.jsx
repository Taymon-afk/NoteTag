...
export default function NotesList() {
  ...
  function addTag(event) {
    ...
    try { createTag(newTag); setNewTag(''); setTagError(''); }
  ...
  return <div className="app-page">
    ...
    <main className="notes-page">
      ...
      {shown.length ? <div className="notes-grid">{shown.map((note) => <Link to={`/notes/${note.id}`} className="note-card" key={note.id}>
        ...
        <h3>{note.title || 'Без названия'}</h3><p className="note-preview">{note.content || 'Пустая заметка'}</p>
